...
public abstract class AbstractCSharpCodegen extends DefaultCodegenConfig {
                    ...
                    if (operation.returnType != null) {
                        ...
                        if (this.collectionTypes.contains(typeMapping)) {
                            ...
                            if (/*this.returnICollection && */(
                                ...
                                if (genericStart > 0) {
                                    operation.returnType = "ICollectionClient" + typeMapping.substring(genericStart);
                                }
                            }
                        } else {
                            operation.returnContainer = operation.returnType;
    ...
    public String apiFilename(String templateName, String tag) {
        ...
        String filename = toApiFilename(tag);

        if (templateName.equalsIgnoreCase("IApi.mustache")) {
            filename = "I" + toApiFilename(tag);
        }
    ...
    public String toApiName(String name) {
            ...
            return getPluralName(initialCaps(name)) + "Client";
    }

    protected String getPluralName(String name) {
        String pluralName = name + "s";
        Character lastChar = name.charAt(name.length() - 1);
