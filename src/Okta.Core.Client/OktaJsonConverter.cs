namespace Okta.Core
{
    ...
    {
        private static TypeInfo ApiObjectTypeInfo = typeof(ApiObject).GetTypeInfo();

        public override bool CanConvert(Type type)
        {
            bool isApiObject = ApiObjectTypeInfo.IsAssignableFrom(type.GetTypeInfo());
            return isApiObject;
        }
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            if (reader.TokenType == JsonToken.StartObject)
            {
                ...
                foreach (PropertyInfo p in objectType.GetTypeInfo().DeclaredProperties)
                {
                    // If member is serializable
                    var jsonAttribute = p.GetCustomAttribute(typeof(JsonPropertyAttribute));
                    if (jsonAttribute != null)
                    {
                        var pTypeInfo = p.PropertyType.GetTypeInfo();
                        ...
                        if (ApiObjectTypeInfo.IsAssignableFrom(pTypeInfo))
                        {
                            v = Utils.DeserializeObject(value.ToString(), p.PropertyType);
                        }

                        // Handle the Link dictionary case
                        else if (typeof(Dictionary<string, List<Link>>).GetTypeInfo().IsAssignableFrom(pTypeInfo))
                        {
        ...
        private JToken WalkObject(object value)
        {
            ...
            var objectType = value.GetType().GetTypeInfo();

            foreach (PropertyInfo member in objectType.DeclaredProperties)
            {
                // If member is serializable
                var jsonAttribute = member.GetCustomAttribute(typeof(JsonPropertyAttribute));
                if (jsonAttribute != null)
                {
                    // Get the mapped attribute name for a field or property
                    var attributeName = (jsonAttribute as JsonPropertyAttribute).PropertyName;
                    ...
                    else if (typeof(DateTime).GetTypeInfo().IsAssignableFrom(pTypeInfo))
                    {
