...
{
    [Cmdlet(VerbsCommon.Reset, "OktaUserFactor")]
    public class ResetOktaUserFactor : OktaCmdlet
    {
        [Parameter(
            Mandatory = true,
            ...
            if (user != null)
            {
                ...
                if (!string.IsNullOrEmpty(FactorId))
                {
                    ...
                    if (userFactor != null)
                    {
                        try
                        {
                            userFactorsClient.Reset(userFactor);
                            WriteObject($"The factor was successfully removed for user {IdOrLogin}.");
                        }
