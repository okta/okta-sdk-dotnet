...
namespace Okta.Sdk.UnitTest.Client
{
    ...
    {
        [Fact]
        public async Task ExpandUrlWhenUpdatingDpopAuthorizationHeader()
        {
            var jsonPrivateKey = @"{
            ...
            tokenReturnQueue.Enqueue("myAccessToken");
            ...
            requestOptions.PathParameters.Add("id", "someResourceId");
            requestOptions.HeaderParameters.Add("DPoP", "myDpopJwt");
            
            await oauthTokenProvider.AddOrUpdateAuthorizationHeader(requestOptions, "resource/{id}", "GET");
            
            requestOptions.HeaderParameters.TryGetValue("DPoP", out var dpopHeader).Should().BeTrue();
            dpopHeader.Should().HaveCount(1);
            
            var decodedJwt = new JwtSecurityTokenHandler().ReadJwtToken(dpopHeader[0]);
