// <copyright file="ApiClientShould.cs" company="Okta, Inc">
...
namespace Okta.Sdk.UnitTest.Client
{
    public class ApiClientShould
    {
        ...
        [Fact]
        public async Task ExpandUrlInDpopHeadersWhenRetrying429Request()
        {
            ...
            configuration.MaxRetries = 2;
            configuration.AuthorizationMode = AuthorizationMode.PrivateKey;
            configuration.OktaDomain = "https://mytestorg.com";

            Queue<string> tokenReturnQueue = new Queue<string>();
            tokenReturnQueue.Enqueue("myAccessToken2");

            var oauthTokenProvider = new DefaultOAuthTokenProvider(configuration, new MockOAuthApi(configuration, returnQueue: tokenReturnQueue, isDpop: true));
            var apiClient = new ApiClient(new OktaApiClientOptions(configuration, oauthTokenProvider));

            var mockHttp = new MockHttpMessageHandler();

            mockHttp
                .When("https://mytestorg.com/resource/someResourceId")
                .Respond(HttpStatusCode.OK, "application/json", "{}");
            ...
            request.Parameters.AddParameter(new HeaderParameter("Authorization", "DPoP myAccessToken"));
            request.Parameters.AddParameter(new HeaderParameter("DPoP", "myDpopJwt"));
            request.Parameters.AddParameter(new UrlSegmentParameter("id", "someResourceId"));
            ...
            dpopHeader = request.Parameters.FirstOrDefault(x => x.Name == "DPoP");
            dpopHeader.Value.Should().NotBe("myDpopJwt");
            dpopHeader.Value.Should().BeAssignableTo(typeof(string));

            var decodedJwt = new System.IdentityModel.Tokens.Jwt.JwtSecurityTokenHandler().ReadJwtToken((string)dpopHeader.Value);
            decodedJwt.Payload["htu"].Should().Be("https://mytestorg.com/resource/someResourceId");
        ...
        public void NotFailOnEmptyProxyConfig()
        {
            ...
            // Use a reliable temp path instead of the current directory
            string testDir = Path.Combine(Path.GetTempPath(), $"okta-test-{Guid.NewGuid()}");
            Directory.CreateDirectory(testDir);
            string appSettingsPath = Path.Combine(testDir, "appsettings.json");
            
            // Save the current directory and change to test directory
            ...
            {
                // Restore original directory (may fail if it's been cleaned up)
                try
                {
                ...
                {
                    // Ignore if we can't restore - directory may have been cleaned up
                }
