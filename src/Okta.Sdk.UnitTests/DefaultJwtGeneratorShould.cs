...
namespace Okta.Sdk.UnitTests
{
    public class DefaultJwtGeneratorShould
    {
        ...
        private static Microsoft.IdentityModel.Tokens.JsonWebKey GetMockRSAPublicKey()
        {
            var jsonPublicKey = @"{""kty"":""RSA"",""e"":""AQAB"",""n"":""mTjMc8AxU102LT1Jf-1qkGmaSiK4L7DDlC1SMvtyCRbDaiJDIagedfp1w8Pgud8YWOaS5FFx0S6JqGGP2U8OtpowzBcv5sYa-e5LHfnoueTJPj_jnI3fj5omZM1w-ofhFLPZoYEQ7DFYw0yLrzf8zaKB5-9BZ8yyOLhSKqxaOl2s7lw2TrwBRuQpPXmEir70oDPvazd8-An5ow6F5q7mzMtHAt61DJqrosRHiRwh4N37zIX_RNu-Tn1aMktCBl01rdoDyVq7Y4iwNH8ZAtT5thKK2eo8d-jb9TF9PH6LGffYCth157w-K4AZwXw74Ybo5NOux3XpIpKRbFTwvBLp1Q""}";

            return new Microsoft.IdentityModel.Tokens.JsonWebKey(jsonPublicKey);
        }

        private static JsonWebKeyConfiguration GetMockECPrivateKeyConfiguration()
        {
            var jsonPrivateKey = @"{
	                                ""kty"": ""EC"",
        ...
        private static Microsoft.IdentityModel.Tokens.JsonWebKey GetMockECPublicKey()
        {
            var jsonPublicKey = @"{
                                    ""kty"": ""EC"",
                                    ""crv"": ""P-256"",
        ...
        public void FailForUnsupportedKty(string kty)
        {
            ...
            var mockPrivateKeyConfiguration = GetMockRSAPrivateKeyConfiguration();
            ...
            Action action = () => new DefaultJwtGenerator().GenerateSignedJWT(configuration);
        ...
        [InlineData("foo")]
        [InlineData("bar")]
        public void FailForInvalidPrivateKey(string kty)
        {
        ...
        public void GenerateRSASignedJWT()
        {
            ...
            var signedJwt = new DefaultJwtGenerator().GenerateSignedJWT(configuration);

            // Verify signature with public key
            var claimsPrincipal = new JwtSecurityTokenHandler().ValidateToken(signedJwt, new TokenValidationParameters
            {
                ValidAudience = $"{configuration.OktaDomain}oauth2/v1/token",
                ValidIssuer = configuration.ClientId,
                IssuerSigningKey = GetMockRSAPublicKey(),
            }, out var parsedToken);
        ...
        {
            var mockPrivateKeyConfiguration = GetMockECPrivateKeyConfiguration();
            ...
            var claimsPrincipal = new JwtSecurityTokenHandler().ValidateToken(signedJwt, new TokenValidationParameters
            {
                ValidAudience = $"{configuration.OktaDomain}oauth2/v1/token",
                ValidIssuer = configuration.ClientId,
                IssuerSigningKey = GetMockECPublicKey(),
            }, out var parsedToken);
