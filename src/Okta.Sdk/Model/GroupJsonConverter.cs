...
namespace Okta.Sdk.Model
{
    ...
    public class GroupJsonConverter : JsonConverter<Group>
    {
        ...
        public override Group ReadJson(JsonReader reader, Type objectType, Group existingValue, bool hasExistingValue, JsonSerializer serializer)
        {
            ...
            {
                group.Type = jsonObject["type"].ToObject<GroupType>(tempSerializer);
            }
            ...
            {
                group.Links = jsonObject["_links"].ToObject<GroupLinks>(tempSerializer);
            }

            // Deserialize Profile using the correct type based on objectClass
            if (jsonObject["profile"] != null)
            {
                ...
                if (isAdGroup)
                {
                    var adProfile = JsonConvert.DeserializeObject<OktaActiveDirectoryGroupProfile>(profileJson);
                    group.Profile = new GroupProfile(adProfile);
                }
                else
                {
                    var userGroupProfile = JsonConvert.DeserializeObject<OktaUserGroupProfile>(profileJson);
                    group.Profile = new GroupProfile(userGroupProfile);
                }
            }
