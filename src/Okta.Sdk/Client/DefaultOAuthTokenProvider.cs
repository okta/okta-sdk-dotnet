...
namespace Okta.Sdk.Client
{
    ...
    public class DefaultOAuthTokenProvider : IOAuthTokenProvider
    {
        ...
        public DefaultOAuthTokenProvider(Configuration configuration, HttpClient httpClient = null, IJwtGenerator jwtGenerator = null, ILogger logger = null, RestClient client = null)
        {
        ...
        private async Task<string> RequestAccessTokenAsync()
        {
            ...
            var accessTokenUri = $@"oauth2/v1/token";
                 //?grant_type=client_credentials&scope={string.Join("+", Configuration.Scopes)}&client_assertion_type=urn:ietf:params:oauth:client-assertion-type:jwt-bearer&client_assertion";

            Okta.Sdk.Client.RequestOptions requestOptions = new Okta.Sdk.Client.RequestOptions();

            requestOptions.PathParameters.Add("grant_type", "client_credentials");
            requestOptions.PathParameters.Add("scope", $"{string.Join("+", Configuration.Scopes)}");
            requestOptions.PathParameters.Add("client_assertion_type", "urn:ietf:params:oauth:client-assertion-type:jwt-bearer");
            requestOptions.PathParameters.Add("client_assertion", $"={jwtSecurityToken.ToString()}");
