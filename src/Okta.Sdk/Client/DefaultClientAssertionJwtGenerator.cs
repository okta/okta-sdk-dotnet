...
namespace Okta.Sdk.Client
{
    ...
    public class DefaultClientAssertionJwtGenerator : IClientAssertionJwtGenerator
    {
        ...
        public DefaultClientAssertionJwtGenerator(IReadableConfiguration configuration)
        {
            ...
            // Credentials supplied directly carry their own algorithm, so there is no kty to check. This is
            // how a key that never leaves a TPM or an HSM is used (see issue #864).
            if (configuration.PrivateKeySigningCredentials == null && !_supportedKeys.Contains(configuration.PrivateKey?.Kty))
            {
        ...
        {
            // Credentials the caller supplied win: the whole point is that the key material behind them is
            // never available as a JWK.
            if (_configuration.PrivateKeySigningCredentials != null)
            {
                return _configuration.PrivateKeySigningCredentials;
            }
