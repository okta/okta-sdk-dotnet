...
//
//         [Fact]
//         public async Task CreateIdpDiscoveryRule()
//         {
//             var guid = Guid.NewGuid();
//
...
//                     Priority = 1,
//                     Name = $"dotnet-sdk: {nameof(CreateIdpDiscoveryRule)} {guid}".Substring(0, 50),
...
//                 createdRule.Type.Should().Be(PolicyRuleType.IDPDISCOVERY);
//                 createdRule.Status.Should().Be(LifecycleStatus.ACTIVE);
//                 ((IdpDiscoveryPolicyRule)createdRule).Actions.Idp.Providers.Any(x => x.Type == IdentityProviderType.OKTA).Should().BeTrue();
//                 ((IdpDiscoveryPolicyRule)createdRule).Conditions.Network.Connection.Should().Be(PolicyNetworkConnection.ZONE);
//                 ((IdpDiscoveryPolicyRule)createdRule).Conditions.Platform.Include.Any(x => x.Os.Type == PolicyPlatformOperatingSystemType.ANY).Should().BeTrue();
//
//             }
//             finally
//             {
...
//
//         [Fact]
//         public async Task GetIdpDiscoveryRule()
//         {
//             var guid = Guid.NewGuid();
//
//             var policies = await _policyApi.ListPolicies(PolicyType.IDPDISCOVERY).ToListAsync();
...
//                     Type = PolicyRuleType.IDPDISCOVERY,
//                     Priority = 1,
//                     Name = $"dotnet-sdk: {nameof(GetIdpDiscoveryRule)} {guid}".Substring(0, 50),
//                     Actions = new IdpPolicyRuleAction
//                     {
//                         Idp = new IdpPolicyRuleActionIdp
//                         {
...
//                 createdRule.Should().NotBeNull();
//
//                 var retrievedRule = await _policyApi.GetPolicyRuleAsync(idpPolicy.Id, createdRule.Id);
//
